...
// Xe URL
const xeURL = "https://api.exchangeratesapi.io/latest?symbols=%s&base=%s&access_key=%s"
...
func (f *Xe) renameSymbol(symbol string) string {
	return strings.ToUpper(symbol)
}

func (f Xe) Pool() query.WorkerPool {
	return f.WorkerPool
}

func (f Xe) PullPrices(pairs []Pair) []FetchResult {
...
func (f *Xe) getURL(base string, quotes []Pair) string {
	symbols := []string{}
	for _, pair := range quotes {
		symbols = append(symbols, f.renameSymbol(pair.Quote))
	}
	return fmt.Sprintf(xeURL, strings.Join(symbols, ","), f.renameSymbol(base), f.APIKey)
}

func (f *Xe) callByBase(base string, pairs []Pair) ([]FetchResult, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to parse XE response: %w", err)
	}
	if resp.Rates == nil {
		return nil, fmt.Errorf("failed to parse XE response: %+v", resp)
	}
