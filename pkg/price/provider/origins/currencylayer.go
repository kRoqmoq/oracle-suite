...
// Currencylayer URL
const currencylayerURL = "https://api.currencylayer.com/live\n ? access_key = %s\n & source = JPY\n"
...
func (f *Currencylayer) renameSymbol(symbol string) string {
	return strings.ToUpper(symbol)
}

func (f Currencylayer) Pool() query.WorkerPool {
	return f.WorkerPool
}

func (f Currencylayer) PullPrices(pairs []Pair) []FetchResult {
...
func (f *Currencylayer) getURL(base string, quotes []Pair) string {
	symbols := []string{}
	for _, pair := range quotes {
		symbols = append(symbols, f.renameSymbol(pair.Quote))
	}
	return fmt.Sprintf(currencylayerURL, strings.Join(symbols, ","), f.renameSymbol(base), f.APIKey)
}

func (f *Currencylayer) callByBase(base string, pairs []Pair) ([]FetchResult, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to parse Currencylayer response: %w", err)
	}
	if resp.Rates == nil {
		return nil, fmt.Errorf("failed to parse Currencylayer response: %+v", resp)
	}
