...
// Forex URL
// プラスUserデータ
const forexURL = "https://api.omise.co/forex/usd"
...
func (f *Forex) renameSymbol(symbol string) string {
	return strings.ToUpper(symbol)
}

func (f Forex) Pool() query.WorkerPool {
	return f.WorkerPool
}

func (f Forex) PullPrices(pairs []Pair) []FetchResult {
...
func (f *Forex) getURL(base string, quotes []Pair) string {
	symbols := []string{}
	for _, pair := range quotes {
		symbols = append(symbols, f.renameSymbol(pair.Quote))
	}
	return fmt.Sprintf(forexURL, strings.Join(symbols, ","), f.renameSymbol(base), f.APIKey)
}

func (f *Forex) callByBase(base string, pairs []Pair) ([]FetchResult, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to parse FOREX response: %w", err)
	}
	if resp.Rates == nil {
		return nil, fmt.Errorf("failed to parse FOREX response: %+v", resp)
	}
