...
// Currencyapi URL
const currencyapiURL = "https://currencyapi.net/api/v1/rates?key=$s&base=$s&output=JSON"
...
func (f *Currencyapi) renameSymbol(symbol string) string {
	return strings.ToUpper(symbol)
}

func (f Currencyapi) Pool() query.WorkerPool {
	return f.WorkerPool
}

func (f Currencyapi) PullPrices(pairs []Pair) []FetchResult {
...
func (f *Currencyapi) getURL(base string, quotes []Pair) string {
	symbols := []string{}
	for _, pair := range quotes {
		symbols = append(symbols, f.renameSymbol(pair.Quote))
	}
	return fmt.Sprintf(currencyapiURL, strings.Join(symbols, ","), f.renameSymbol(base), f.APIKey)
}

func (f *Currencyapi) callByBase(base string, pairs []Pair) ([]FetchResult, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to parse Currencyapi response: %w", err)
	}
	if resp.Rates == nil {
		return nil, fmt.Errorf("failed to parse Currencyapi response: %+v", resp)
	}
